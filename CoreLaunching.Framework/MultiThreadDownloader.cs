    ...
    public class ThreadDownloader
    {
        ...
        public ThreadDownloader(string url, int ThreadNum, string LocalName)
        {
            ...
            for (int i = 0; i < threadNum; i++)
            {
                ...
                if (CoreLaunching.LoaclPath.TmpFolderPath == null)
                {
                    throw new System.NullReferenceException();
                }
                TmpFileNames[i] = Path.Combine(CoreLaunching.LoaclPath.TmpFolderPath, i.ToString() + ".tmp");
                if (i < threadNum - 1)
                {
        ...
        public void Download()
        {
            ...
            t.AutoReset = true;
            t.Start();
            Thread ComeAlong = new Thread(new ThreadStart(new HebinFile(this).Hbfile));
            ComeAlong.Start();
        }
        #endregion
        public class HebinFile
        {
            ThreadDownloader args;
            public HebinFile(ThreadDownloader Args)
            {
                args = Args;
            }
            public void Hbfile()
            {
                ...
                {
                    if (Directory.Exists(Path.GetDirectoryName(args.localName)) == false)
                    {
